...
@endpush
@push('title')
    Kepala Dinas | Pengaturan Aplikasi | SIMEGAL
@endpush
@push('description')
    Kepala Dinas | Pengaturan Aplikasi | Sistem Informasi Metrologi Legal Pemerintah Kabupaten Tangerang
@endpush
@push('header-title')
    Kepala Dinas | Pengaturan Aplikasi
@endpush
        ...
        <div id="kt_toolbar_container" class="container-xxl d-flex flex-stack flex-wrap">
            ...
            <div class="page-title d-flex flex-column me-3">
                ...
                <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 pt-1">
                    ...
                    {{-- end::Item --}}
                    {{-- begin::Item --}}
                    <li class="breadcrumb-item text-dark">Kepala Dinas</li>
                    {{-- end::Item --}}
                </ul>
                {{-- end::Breadcrumb --}}
            </div>
            ...
            @if (\CID::subRoleAdmin() == true)
                ...
                <div class="d-flex align-items-center py-2 py-md-1">
                    <a href="javascript:void(0);" class="btn btn-info fw-bold" data-bs-toggle="modal" data-bs-target="#add_kadis" data-mode="add"><i class="fa-solid fa-plus"></i>Tambah Kepala Dinas</a>
                </div>
                {{-- end::Actions --}}
            @endif
        </div>
...
@push('modals')
    ...
    <div class="modal fade add" tabindex="-1" id="add_kadis">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="{{ route('set.apps.kadis.store') }}" method="POST" id="formKadis" enctype="multipart/form-data">
                    @csrf
...
@push('scripts')
    ...
    <script>
        var table = $('#datatable').DataTable({
            ...
            "ajax": "{!! route('set.apps.kadis.data') !!}",
            "columns": [{
                    data: 'DT_RowIndex',
                    name: 'DT_RowIndex'
                },
        ...
        $(document).on('click', "[data-delete]", function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{!! route('set.apps.kadis.destroy') !!}",
                        type: 'POST',
                        data: {
                            uuid: uuid,
                            _method: 'delete',
        ...
        $(document).on('click', "[data-status]", function() {
            ...
            $.ajax({
                url: "{!! route('set.apps.kadis.status') !!}",
                type: 'POST',
                data: {
                    uuid: uuid,
                    status: status,
    ...
    {{-- edit form --}}
    <script>
        $('a[data-bs-target^="#add_kadis"]').click(function() {
            var mode = $(this).data("mode");
            if (mode == "add") {
                $("#modal_title").html("Form Tambah Kadis");
                $("#formKadis").trigger("reset");
            }
        });
    </script>
