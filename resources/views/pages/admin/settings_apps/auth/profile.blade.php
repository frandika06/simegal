...
@endpush
@push('title')
    Profile {{ $data->nama_lengkap }} | SIMEGAL
@endpush
@push('description')
    Profile {{ $data->nama_lengkap }} | Sistem Informasi Metrologi Legal Pemerintah Kabupaten Tangerang
@endpush
@push('header-title')
            ...
            <div class="d-flex flex-column flex-lg-row">
                        ...
                        <div class="card-body">
                            ...
                            <div class="d-flex flex-center flex-column py-5">
                                ...
                                <div class="symbol symbol-100px symbol-circle mb-7">
                                    <img src="{{ \CID::pp($data->foto) }}" alt="{{ $data->nama_lengkap }}" />
                                </div>
                                {{-- end::Avatar --}}
                                {{-- begin::Name --}}
                ...
                <div class="flex-lg-row-fluid ms-lg-15">
                    ...
                    <div class="tab-content" id="myTabContent">
                        {{-- path-profile::begin --}}
                        @include('pages.admin.settings_apps.auth.path_profile')
                        {{-- path-profile::end --}}
                        {{-- path-keamanan::begin --}}
                        @include('pages.admin.settings_apps.auth.path_keamanan')
                        {{-- path-keamanan::end --}}
                        {{-- path-log::begin --}}
                        @include('pages.admin.settings_apps.auth.path_log')
                        {{-- path-log::end --}}
                    </div>
                    {{-- end:::Tab content --}}
    ...
    <script>
        $(document).ready(function() {
            ...
            $(".path-tab").click(function() {
                ...
                localStorage.setItem('tabsProfilePegawaiAktif', href);
            })
        });

        function getTabsProfil() {
            var tabsProfilePegawaiAktif = localStorage.getItem('tabsProfilePegawaiAktif');
            if (tabsProfilePegawaiAktif === null) {
                localStorage.setItem('tabsProfilePegawaiAktif', '#profile_tab_path_profile');
                ...
                $(".tabs-profile>li").each(function(index) {
                    if ($(this).find('a[href^="' + tabsProfilePegawaiAktif + '"]')) {
                        $('a[href^="' + tabsProfilePegawaiAktif + '"]').addClass(" active");
                        $(tabsProfilePegawaiAktif).addClass(" show active");
                    }
                });
            }
        }
