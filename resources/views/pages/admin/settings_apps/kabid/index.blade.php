...
@endpush
@push('title')
    Kepala Bidang | Pengaturan Aplikasi | SIMEGAL
@endpush
@push('description')
    Kepala Bidang | Pengaturan Aplikasi | Sistem Informasi Metrologi Legal Pemerintah Kabupaten Tangerang
@endpush
@push('header-title')
    Kepala Bidang | Pengaturan Aplikasi
@endpush
        ...
        <div id="kt_toolbar_container" class="container-xxl d-flex flex-stack flex-wrap">
            ...
            <div class="page-title d-flex flex-column me-3">
                ...
                <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 pt-1">
                    ...
                    {{-- end::Item --}}
                    {{-- begin::Item --}}
                    <li class="breadcrumb-item text-dark">Kepala Bidang</li>
                    {{-- end::Item --}}
                </ul>
                {{-- end::Breadcrumb --}}
            </div>
            ...
            @if (\CID::subRoleAdmin() == true)
                ...
                <div class="d-flex align-items-center py-2 py-md-1">
                    <a href="javascript:void(0);" class="btn btn-info fw-bold" data-bs-toggle="modal" data-bs-target="#add_kabid" data-mode="add"><i class="fa-solid fa-plus"></i>Tambah Kepala Bidang</a>
                </div>
                {{-- end::Actions --}}
            @endif
        </div>
...
@push('modals')
    ...
    <div class="modal fade add" tabindex="-1" id="add_kabid">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="{{ route('set.apps.kabid.store') }}" method="POST" id="formKabid" enctype="multipart/form-data">
                    @csrf
...
@push('scripts')
    ...
    <script>
        var table = $('#datatable').DataTable({
            ...
            "ajax": "{!! route('set.apps.kabid.data') !!}",
            "columns": [{
                    data: 'DT_RowIndex',
                    name: 'DT_RowIndex'
                },
        ...
        $(document).on('click', "[data-delete]", function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{!! route('set.apps.kabid.destroy') !!}",
                        type: 'POST',
                        data: {
                            uuid: uuid,
                            _method: 'delete',
        ...
        $(document).on('click', "[data-status]", function() {
            ...
            $.ajax({
                url: "{!! route('set.apps.kabid.status') !!}",
                type: 'POST',
                data: {
                    uuid: uuid,
                    status: status,
    ...
    {{-- edit form --}}
    <script>
        $('a[data-bs-target^="#add_kabid"]').click(function() {
            var mode = $(this).data("mode");
            if (mode == "add") {
                $("#modal_title").html("Form Tambah Kabid");
                $("#formKabid").trigger("reset");
            }
        });
    </script>
