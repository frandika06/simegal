{{-- begin:::Tab pane --}}
<div class="tab-pane fade" id="profile_tab_status_diproses" role="tabpanel">
    {{-- begin::Card --}}
    <div class="card pt-4 mb-6 mb-xl-9">
        ...
        <div class="card-body pt-0 pb-5">
            ...
            <div class="table-responsive">
                {{-- begin::Table --}}
                <table id="datatableDiproses" class="table table-striped table-hover table-row-bordered gy-5 gs-7 border rounded">
...
@push('scripts')
    ...
    <script>
        $('[name="q_tahun"]').change(function() {
            $('#datatableDiproses tbody').empty();
            tableDiproses.ajax.reload(null, true);
            getStatistikPermohonan();
        });

        var tableDiproses = $('#datatableDiproses').DataTable({
            "select": false,
            "paging": true,
            "searching": true,
            "ordering": true,
        ...
        $(document).on('click', "[data-selesai]", function() {
            ...
            Swal.fire({
                ...
                confirmButtonText: "Iya, Permohonan Selesai!",
        ...
        $(document).on('click', "[data-reset]", function() {
            let uuid = $(this).attr('data-reset');
            let status = $(this).attr('data-status');
            Swal.fire({
                title: "Reset Permohonan",
                text: "Apakah Anda Yakin?",
                icon: "warning",
                showCancelButton: true,
                confirmButtonText: "Iya, Reset Permohonan!",
                cancelButtonText: 'Tidak, Batalkan!',
            }).then((result) => {
                if (result.isConfirmed) {
                    $.ajax({
