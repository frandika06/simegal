...
class ScdTinjutUapvController extends Controller
{
    /**
    ...
    public function index(Request $request)
    {
        ...
        if ($request->session()->exists('filter_tahun')) {
            ...
            $tags = $request->session()->get('filter_tags_tinjut_uapv');
        } else {
            $request->session()->put('filter_tahun', date('Y'));
            $request->session()->put('filter_tags_tinjut_uapv', 'Tera');
            $tahun = date('Y');
            $tags = "Tera";
        }
        ...
        $tahunPermohonan = PermohonanPeneraan::select(DB::raw("YEAR(created_at) year"))
            ...
            ->orderBy("year", "DESC")
            ->get();
        return view('pages.admin.schedule_apps.tindak_lanjut.uapv.index', compact(
            'tahun',
            'tahunPermohonan',
        ));
    }
    ...
    public function data(Request $request)
    {
        ...
        if ($request->ajax()) {
            if (isset($_GET['filter'])) {
                ...
                $request->session()->put('filter_tags_tinjut_uapv', $tags);
            } else {
                $tahun = date('Y');
                $tags = "Tera";
            }
