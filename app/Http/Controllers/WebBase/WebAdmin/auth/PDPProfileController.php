...
class PDPProfileController extends Controller
{
    ...
    public function index()
    {
        ...
        $logs_login = SysLogin::whereUuidProfile($uuid_profile)->orderBy("created_at", "DESC")->limit(10)->get();
        $logs_aktifitas = SysLogAktifitas::whereUuidProfile($uuid_profile)->orderBy("created_at", "DESC")->limit(10)->get();
    ...
    private function updateKeamanan($request)
    {
        ...
        if ($save_1) {
            ...
            CID::addToLogAktifitas($request, $log);
            // alert success
            alert()->success('Berhasil!', 'Berhasil Mengubah Keamanan Akun, Silahkan Anda Login Kembali.');
            return back();
        } else {
