...
namespace App\Http\Controllers\WebBase\WebAdmin\SettingsApps\kadis;
...
class SetAppsKadisController extends Controller
{
    /**
    ...
    {
        return view('pages.admin.settings_apps.kadis.index');
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($save_1 && $save_2) {
            ...
            $log = [
                "apps" => "Settings Apps",
                "subjek" => "Menambahkan Akun Kepala Dinas: " . $request->nama_lengkap . " - " . $uuid_pegawai,
                "aktifitas" => $aktifitas,
                "device" => "web",
                "dashboard" => "1",
            ];
            ...
            // alert success
            alert()->success('Berhasil!', "Berhasil Menambahkan Kepala Dinas: " . $request->nama_lengkap);
            return back();
        } else {
            alert()->error('Gagal!', "Gagal Menambahkan Kepala Dinas: " . $request->nama_lengkap);
    ...
    public function edit(Request $request, $enc_uuid)
    {
        ...
        $logs_aktifitas = SysLogAktifitas::whereUuidProfile($uuid_profile)
            ...
            ->orderBy("created_at", "DESC")
            ->get();
        return view('pages.admin.settings_apps.kadis.edit_profile.profile', compact(
            'enc_uuid',
            'data',
            'logs_login',
            'logs_aktifitas',
    ...
    private function updateProfile($request, $uuid)
    {
        ...
        if ($save_1) {
            ...
            $log = [
                "apps" => "Settings Apps",
                "subjek" => "Berhasil Mengubah Profile Kepala Dinas: " . $request->nama_lengkap . " - " . $uuid,
                "aktifitas" => $aktifitas,
                "device" => "web",
                "dashboard" => "1",
            ];
            CID::addToLogAktifitas($request, $log);
            // alert success
            alert()->success('Berhasil!', "Berhasil Mengubah Profile Kepala Dinas: " . $request->nama_lengkap);
            return back();
        } else {
            alert()->error('Gagal!', "Gagal Mengubah Profile Kepala Dinas: " . $request->nama_lengkap);
            return back()->withInput($request->all());
        }
    }
    // update keamanan
    private function updateKeamanan($request, $uuid_pegawai)
    {
        ...
        if ($save_1) {
            ...
            $log = [
                "apps" => "Settings Apps",
                "subjek" => "Berhasil Mengubah Akun Login Kepala Dinas " . $data->nama_lengkap . " - " . $data->uuid,
                "aktifitas" => $aktifitas,
                "device" => "web",
                "dashboard" => "1",
            ];
    ...
    public function show(Request $request, $enc_uuid)
    {
        ...
        $logs_aktifitas = SysLogAktifitas::whereUuidProfile($uuid_profile)
            ...
            ->orderBy("created_at", "DESC")
            ->get();
        return view('pages.admin.settings_apps.kadis.view_profile.profile', compact(
            'enc_uuid',
            'data',
            'logs_login',
            'logs_aktifitas',
    ...
    public function destroy(Request $request)
    {
        ...
        if ($save_1) {
            ...
            $log = [
                "apps" => "Settings Apps",
                "subjek" => "Berhasil Menghapus Kepala Dinas: " . $data->nama_lengkap . " - " . $uuid,
                "aktifitas" => $aktifitas,
                "device" => "web",
                "dashboard" => "1",
            ];
    ...
    public function status(Request $request)
    {
        ...
        if ($save_1) {
            ...
            $log = [
                "apps" => "Settings Apps",
                "subjek" => "Mengubah Status Kepala Dinas: " . $data->RelPegawai->nama_lengkap . " - " . $uuid,
                "aktifitas" => $aktifitas,
                "device" => "web",
                "dashboard" => "1",
            ];
        ...
        if ($request->ajax()) {
            return Datatables::of($data)
                ...
                ->addColumn('aksi', function ($data) {
                    ...
                    $subRoleAdmin = CID::subRoleAdmin();
                    $edit = route('set.apps.kadis.edit', [$enc_uuid]);
                    $show = route('set.apps.kadis.show', [$enc_uuid]);
                    if ($subRoleAdmin == true) {
