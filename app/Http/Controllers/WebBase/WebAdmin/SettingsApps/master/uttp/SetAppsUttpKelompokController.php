...
class SetAppsUttpKelompokController extends Controller
{
    /**
    ...
    public function store(Request $request)
    {
        ...
        // save
        $save_1 = MasterKelompokUttp::create($value_1);
        if ($save_1) {
            // create log
            $aktifitas = [
                "tabel" => array("master_kelompok_uttp"),
                "uuid" => array($uuid),
                "value" => array($value_1),
            ];
            $log = [
                "apps" => "Settings Apps",
                "subjek" => "Menambahkan Master UTTP - Kelompok UTTP: " . $request->nama_kelompok . " - " . $uuid,
                "aktifitas" => $aktifitas,
                "device" => "web",
                "dashboard" => "0",
            ];
            ...
            // alert success
            alert()->success('Berhasil!', "Berhasil Menambahkan Master UTTP - Kelompok UTTP: " . $request->nama_kelompok);
            return back();
        } else {
            alert()->error('Gagal!', "Gagal Menambahkan Master UTTP - Kelompok UTTP: " . $request->nama_kelompok);
    ...
    public function show($enc_uuid)
    {
        ...
        $data = MasterKelompokUttp::findOrFail($uuid);

        $title = "Detail Kelompok UTTP";
        return view('pages.admin.settings_apps.master.uttp.kelompok_uttp.view', compact(
            'enc_uuid',
            'title',
            'data',
        ));
    ...
    public function edit($enc_uuid)
    {
        ...
        $title = "Edit Kelompok UTTP";
        $submit = "Simpan";
        return view('pages.admin.settings_apps.master.uttp.kelompok_uttp.create_edit', compact(
            'enc_uuid',
            'title',
            'submit',
            'data',
    ...
    public function update(Request $request, $enc_uuid)
    {
        ...
        if ($save_1) {
            ...
            $log = [
                "apps" => "Settings Apps",
                "subjek" => "Mengubah Master UTTP - Kelompok UTTP: " . $request->nama_kelompok . " - " . $uuid,
                "aktifitas" => $aktifitas,
                "device" => "web",
                "dashboard" => "0",
            ];
            CID::addToLogAktifitas($request, $log);
            // alert success
            alert()->success('Berhasil!', "Berhasil Mengubah Master UTTP - Kelompok UTTP: " . $request->nama_kelompok);
            return back();
        } else {
            alert()->error('Gagal!', "Gagal Mengubah Master UTTP - Kelompok UTTP: " . $request->nama_kelompok);
            return back()->withInput($request->all());
        }
    }
    ...
    public function destroy(Request $request)
    {
        ...
        if ($save_1) {
            ...
            $log = [
                "apps" => "Settings Apps",
                "subjek" => "Berhasil Menghapus Master UTTP - Kelompok UTTP: " . $data->nama_pelayanan . " - " . $uuid,
                "aktifitas" => $aktifitas,
                "device" => "web",
                "dashboard" => "1",
            ];
    ...
    public function status(Request $request)
    {
        ...
        if ($save_1) {
            ...
            $log = [
                "apps" => "Settings Apps",
                "subjek" => "Mengubah Status Master UTTP - Kelompok UTTP: " . $data->nama_pelayanan . " - " . $uuid,
                "aktifitas" => $aktifitas,
                "device" => "web",
                "dashboard" => "1",
            ];
        ...
        if ($request->ajax()) {
            return Datatables::of($data)
                ...
                ->addColumn('aksi', function ($data) {
                    ...
                    $subRoleAdmin = CID::subRoleAdmin();
                    $edit = route('set.apps.mst.uttp.klpk.edit', [$enc_uuid]);
                    $show = route('set.apps.mst.uttp.klpk.show', [$enc_uuid]);
                    if ($subRoleAdmin == true) {
